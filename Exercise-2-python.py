...
weatherDataFrame: DataFrame  = spark.read  \
  .option("header", "true") \
...
minTemp: float = weatherDataFrame.agg(functions.min('temperaturemin')).collect()[0][0]
maxTemp: float = weatherDataFrame.agg(functions.max('temperaturemax')).collect()[0][0]
...
weatherDataFrameWithYear: DataFrame = weatherDataFrame.withColumn('year',functions.year(functions.column('date')))
...
aggregatedDF: DataFrame = weatherDataFrameWithYear.groupBy("year").agg(
    functions.min("temperaturemin").alias("min_temp"),
    functions.max("temperaturemax").alias("max_temp")
).orderBy(functions.desc("year"))
...
task6DF: DataFrame = weatherDataFrameWithYear.groupBy("year").agg(
    functions.min("temperaturemin").alias("min_temp"),
    functions.max("temperaturemax").alias("max_temp"),
    functions.count("year").alias("entries"),
    functions.round(functions.mean("avgwindspeed"),2).alias("avg_windspeed")
...
min2012: float = task6DF.where(task6DF.year == 2012).select("min_temp").collect()[0][0]
max2016: float = task6DF.where(task6DF.year == 2016).select("max_temp").collect()[0][0]
entries2018: int = task6DF.where(task6DF.year == 2018).select("entries").collect()[0][0]
wind2008: float = task6DF.where(task6DF.year == 2008).select("avg_windspeed").collect()[0][0]
print(min2012)
print(max2016)
print(entries2018)
print(wind2008)
...
yearWithMostDaysWithFog: int = weatherDataFrameWithYear.groupBy("year").agg(
    functions.count_if(weatherDataFrameWithYear.fog == True).alias("fog")
).orderBy(functions.desc("fog")).collect()[0][0]

print(yearWithMostDaysWithFog)
