...
df_csv = spark.read.option("header", "true").option("delimiter", "|").csv(source_csv_folder)
...
df_csv.coalesce(1).write.format("csv").option("delimiter", "|").option("header", "true").save(target_file_csv)
...
df_new_rows = df_csv.filter(df_csv.City =="New York").sort("Start_Time", ascending=False).limit(42)
df_new_rows.printSchema()
df_new_rows = df_new_rows.withColumn("ID", functions.concat(functions.col("ID"), functions.lit("_Z1")))
df_new_rows.show(2)

df_parq = spark.read.parquet(source_parquet_folder)
df_new_rows_parquet  = (
    df_parq
    .filter(functions.col("City") == "New York")
    .orderBy(functions.col("Start_Time").desc())
    .limit(more_rows_count)
    .withColumn("ID", functions.concat(functions.col('ID'),functions.lit('_Z1')))
)
print(df_new_rows.count())
print(df_csv.count())

print(df_new_rows_parquet.count())
df_new_rows_parquet.printSchema()
...
df_new_rows.write.option("delimiter", "|").option("header", "true").mode("append").csv(target_file_csv)

# Append the new rows to Parquet storage:
df_new_rows_parquet.write.mode("append").parquet(target_file_parquet)


# COMMAND ----------

# Read the merged data from the CSV files to check that the new rows have been stored
df_new_csv = spark.read.option("delimiter", "|").option("header", "true").csv(target_file_csv)
...
df_modified = df_new_rows.withColumn("ID", functions.regexp_replace(functions.col("ID"), "_Z1$", "_Z2"))
df_modified = df_modified.withColumn("AddedColumn1", functions.concat(functions.lit("prefix-"), functions.col("City"))).withColumn("AddedColumn2", functions.lit("New column"))
df_modified = df_modified.drop("Description")

df_modified.show(2)

df_modf_parq = df_new_rows_parquet.withColumn("ID", functions.regexp_replace(functions.col("ID"), "_Z1$", "_Z2"))
df_modf_parq = df_modf_parq.withColumn("AddedColumn1", functions.concat(functions.lit("prefix-"), functions.col("City"))).withColumn("AddedColumn2", functions.lit("New column"))
df_modf_parq = df_modf_parq.drop("Description")
...
# Append the new rows to Parquet storage:
df_modf_parq.write.mode("append").parquet(target_file_parquet)
...
# Read the merged data from the CSV files to check that the new rows have been stored
df_new_csv = spark.read.option("header", "true").option("delimiter", "|").csv(target_file_csv)

# Read the merged data from the Parquet files to check that the new rows have been stored
df_new_parquet = spark.read.parquet(target_file_parquet)
print(df_new_parquet.count())

# COMMAND ----------

# CSV should have been broken in some way
# Read in the CSV data again from the CSV storage: target_file_csv
modified_csv_df =  spark.read.option("header", "true").option("delimiter", "|").csv(target_file_csv)
...
# MAGIC %md
# MAGIC I got similar output for the data in CSV storage.
# MAGIC CSV seems broken and it can be explained with changed number of columns.
# MAGIC For Parquet i got alternative 2.
# MAGIC Same guess for Parquet case as for CSV case.
# MAGIC
# MAGIC Additional observation:
# MAGIC At first I made the modified dataframes from CSV and it couldnt work with Parquet or with delta.
# MAGIC Explanation for this was that parquet couldnt work with changed values of datatype. In CSV all data are in String format but Parquet has different datatypes.
...
# MAGIC |A-3877306_Z2|2023-02-16T20:48:...|2023-02-16T22:00:...|            New York|  Unknown|      NY| 58.0|prefix-New York|
# MAGIC |A-4076884_Z2|2023-02-12T07:54:...|2023-02-12T10:05:...|            New York|  Unknown|      NY| 40.0|prefix-New York|
# MAGIC +------------+--------------------+--------------------+--------------------+---------+--------+-----+---------------+
# MAGIC
...
# MAGIC |A-3877306_Z1|2023-02-16 20:48:16|2023-02-16 22:00:00|Closed road from ...| New York|New York|   NY|         58.0|
# MAGIC |A-4076884_Z1|2023-02-12 07:54:11|2023-02-12 10:05:18|Port Authority NY...| New York|New York|   NY|         40.0|
# MAGIC |A-3877306_Z2|2023-02-16 20:48:16|2023-02-16 22:00:00|                NULL| New York| Unknown|   NY|         58.0|
# MAGIC |A-4076884_Z2|2023-02-12 07:54:11|2023-02-12 10:05:18|                NULL| New York| Unknown|   NY|         40.0|
# MAGIC +------------+-------------------+-------------------+--------------------+---------+--------+-----+-------------+
# MAGIC
...
df_new_rows_parquet.write.mode("append").format("delta").save(target_file_delta)

# Append the new rows using the modified schema, df_modified, to the Delta storage:
df_modf_parq.write.mode("append").format("delta").save(target_file_delta)
...
cleanTargetFolder(delta_table_file)
