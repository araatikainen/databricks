...
dataDF: DataFrame = spark.createDataFrame(dataRDD, ['X', 'label'])
...
print("dataDF size: ", dataDF.count())
...
data: List[Row] = [
    Row(-1.5), Row(2.5), Row(8.5), Row(31.4)
]
dataRdd: RDD[Row] = spark.sparkContext.parallelize(data)
dataDf: DataFrame = spark.createDataFrame(dataRdd, ['X'])
dataDf.show()

vectorAssembler = VectorAssembler(inputCols=['X'], outputCol='features')
assembledDf = vectorAssembler.transform(dataDf)

preds = lrModel.transform(assembledDf)
preds.show()
...
path = 'abfss://shared@tunics320f2023gen2.dfs.core.windows.net/exercises/ex4/'
names = ['aaltonen', 'design', 'verifying']

articlesRdd: RDD[str] = [
    spark.sparkContext.textFile(path + names[0] + ".txt"),
    spark.sparkContext.textFile(path + names[1] + ".txt"),
    spark.sparkContext.textFile(path + names[2] + ".txt")
]
articlesRdd = spark.sparkContext.union(articlesRdd)
...
words = articlesRdd.flatMap(lambda x: x.split(' '))
print(words.count())

# COMMAND ----------

numberOfWords: int = words.count()
print(f"numberOfWords: {numberOfWords}")

numberOfCharacters: int = words.map(lambda x: len(x)).sum()
...
words6 = words.filter(lambda x: len(x)==6)

words6Count: int = words6.count()
print(f"6-letter words: {words6Count}")

counter = collections.Counter(words6.collect())
mostCommonWord = counter.most_common(1)
#print(mostCommonWord)

commonWord: str = mostCommonWord[0][0]
commonWordCount: int = mostCommonWord[0][1]
